...
interface PropertyData {
  ...
  // Legacy fields and image support
  profileImage?: string;
...
function mapToBetterPropertyStructure(rawData: any, images: { [key: string]: string }): PropertyData {
  ...
  // Basic mappings - from new BCAT structure
  data.title = rawData.title || rawData.alertDialogTitle || rawData.propertyName || "";
  data.description = rawData.description || rawData.alertDialogDescription || rawData.hotelRoomDescription || "";
  ...
  // Rules - handle new format from BCAT
  if (rawData.rules && typeof rawData.rules === 'object') {
    data.rules = {
      smokingallowed: rawData.rules.smokingallowed || rawData.rules.smoking || 'No',
      petsallowed: rawData.rules.petsallowed || rawData.rules.pets || 'No',
      childrenallowed: rawData.rules.childrenallowed || rawData.rules.children || 'No',
      partieseventsallowed: rawData.rules.partieseventsallowed || rawData.rules.parties || 'No',
      additionalRules: rawData.rules.additionalRules || rawData.rules.additional || ""
    };
  } else {
    // Try to extract rules from flat structure
    data.rules = {
      smokingallowed: rawData.smokingallowed || 'No',
      petsallowed: rawData.petsallowed || 'No',
      childrenallowed: rawData.childrenallowed || 'No',
      partieseventsallowed: rawData.partieseventsallowed || 'No',
  ...
  // Handle images
  data.alertDialogImage = images.mainImage || images.alertDialogImage || images.image1;
  ...
  Object.keys(images).forEach(key => {
    ...
    if (lowerKey.includes('roomphoto')) {
      data.roomPhotos!.push(images[key]);
    } else if (lowerKey.includes('hotelphoto')) {
      data.hotelPhotos!.push(images[key]);
    } else if (lowerKey.includes('facilitiesphoto')) {
      data.facilitiesPhotos!.push(images[key]);
    }
  });
  
  // If no categorized photos but we have counts, indicate they exist
  if (rawData.roomPhotosCount && data.roomPhotos!.length === 0) {
    // Photos exist but not loaded yet
    for (let i = 0; i < rawData.roomPhotosCount; i++) {
      data.roomPhotos!.push('/api/placeholder/400/300');
    }
  }
                ...
                <div className="mb-2">
                  ...
                  <div className="ml-8 space-y-1 text-sm">
                    {data.rules.smokingallowed && (
                      <div>Smoking: {data.rules.smokingallowed === 'Yes' ? "✅ Allowed" : "❌ Not Allowed"}</div>
                    )}
                    {data.rules.petsallowed && (
                      <div>Pets: {data.rules.petsallowed === 'Yes' ? "✅ Allowed" : "❌ Not Allowed"}</div>
                    )}
                    {data.rules.childrenallowed && (
                      <div>Children: {data.rules.childrenallowed === 'Yes' ? "✅ Allowed" : "❌ Not Allowed"}</div>
                    )}
                    {data.rules.partieseventsallowed && (
                      <div>Parties: {data.rules.partieseventsallowed === 'Yes' ? "✅ Allowed" : "❌ Not Allowed"}</div>
                    )}
  ...
  return (
    <Dialog>
      ...
      <DialogTrigger
        ...
        className='flex justify-center align-items-center w-[250px] h-[330px] flex-col overflow-hidden border border-zinc-950/10 dark:border-zinc-50/10 bg-[#121212] hover:shadow-lg transition-all duration-300'
      >
        {/* Container with relative positioning */}
        <div className='relative w-full h-full'>
          {/* Image */}
          <img
            src={data?.alertDialogImage || data?.roomPhotos?.[0] || '/api/placeholder/400/300'}
            alt={data?.title || data?.propertyName || 'Property'}
        ...
        <div className='flex flex-col justify-end p-2'>
          <DialogTitle className='text-white text-lg font-semibold'>
            {data?.title || data?.propertyName || 'Property'}
          </DialogTitle>
          <DialogSubtitle className='text-sm text-zinc-200'>
            {data?.description?.substring(0, 100) || data?.hotelRoomDescription?.substring(0, 100) || 'Property description'}
          </DialogSubtitle>
          ...
          >
            ...
            {imageCategories.length > 0 && showImages && (
              <div className='relative'>
                <img
                  src={imageCategories[activeImageTab].images[currentImageIndex] || '/api/placeholder/750/400'}
                  alt={`Property ${currentImageIndex + 1}`}
                  className='h-[400px] w-full object-cover rounded-t-lg transition-opacity duration-500 ease-in-out'
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = '/api/placeholder/750/400';
                  }}
                />
                <div className='absolute bottom-1 left-1 text-white text-sm bg-black bg-opacity-50 px-2 py-1 rounded'>
                  Price TBD
            ...
            <div className="px-4 mb-4">
              <h1 className="text-white text-3xl font-semibold mb-2">
                {data?.propertyName || data?.title || "Property"}
              </h1>
              {(data?.hotelRoomDescription || data?.description) && (
