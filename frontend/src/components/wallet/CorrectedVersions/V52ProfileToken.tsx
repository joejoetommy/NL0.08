...
// V52 Building into being a TXID profile data display component + storage of the TXID'S
        ...
        >
          ...
          {inscriptionType === 'display' && (
            <div>
              ...
              {/* Display filtered transactions     'display' && (     */}
                  ...
                  transactionHistory
                    ...
                    .map(tx => (
                      <div key={tx.txid} className="bg-gray-800 rounded-lg p-4">
                        ...
                        {displayTab === 'text' && tx.data && (
                          <div>
                            ...
                            <div className="text-xs text-gray-400">
                              ...
                              <a
                                href={`https://${network === 'testnet' ? 'test.' : ''}whatsonchain.com/tx/${tx.txid}`}
                                target="_blank"
                                rel="noopener noreferrer"
                        ...
                        {/* Image Display */}
                        {displayTab === 'image' && tx.data && (
                          <div>
                            <div className="bg-gray-900 rounded p-2 mb-2">
                              <p className="text-sm text-white">{tx.data.fileName}</p>
                              <p className="text-xs text-gray-400">
                                {(tx.data.fileSize / 1024).toFixed(1)}KB • {tx.data.fileType}
                              </p>
                            </div>
                            <div className="text-xs text-gray-400">
                              <p>{new Date(tx.timestamp).toLocaleString()}</p>
                        ...
                        {/* Profile Display */}
                        {displayTab === 'profile' && tx.data && (
                          <div>
                            <div className="flex items-start gap-3">
                              {tx.data.hasAvatar && (
                                <div className="w-12 h-12 bg-purple-600 rounded-full flex items-center justify-center">
                                  <span className="text-white font-bold">
                                    {tx.data.username?.charAt(0).toUpperCase() || '?'}
                                  </span>
                                </div>
                              )}
                              <div className="flex-1">
                                <h4 className="text-white font-medium">{tx.data.username}</h4>
                                <p className="text-sm text-purple-400">{tx.data.title}</p>
                                <p className="text-sm text-gray-300 mt-1">{tx.data.bio}</p>
                              </div>
                            </div>
                        ...
                        {/* Profile2 Display */}
                        {displayTab === 'profile2' && tx.data && (
                          <div>
                            {tx.data.hasBackground && (
                              <div className="h-24 bg-gradient-to-r from-purple-600 to-pink-600 rounded-t-lg -mx-4 -mt-4 mb-3"></div>
                            )}
                            <div className="flex items-start gap-3">
                              {tx.data.hasAvatar && (
                                <div className="w-12 h-12 bg-purple-600 rounded-full flex items-center justify-center border-2 border-gray-800">
                                  <span className="text-white font-bold">
                                    {tx.data.username?.charAt(0).toUpperCase() || '?'}
                                  </span>
                                </div>
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex gap-2 mt-2">
                                  {tx.data.hasAvatar && (
                                    <span className="text-xs px-2 py-1 bg-gray-700 rounded">Avatar</span>
                                  )}
                                  {tx.data.hasBackground && (
                                    <span className="text-xs px-2 py-1 bg-gray-700 rounded">Background</span>
                                  )}
                                </div>
                              </div>
                            </div>
