...
export const ProfileToken: React.FC = () => {
  ...
  const encryptCurrentData = async () => {
    ...
    try {
      ...
      if (inscriptionType === 'text') {
        ...
        // ENCRYPTED IMAGE COMPRESSION LIMIT: 2.4MB
        // For encrypted images, compress to 2.4MB before encryption
        const base64Data = await imageToBase64(imageFile, undefined, true, 2.4);
        ...
        if (profileImageFile) {
          ...
          // ENCRYPTED PROFILE COMPRESSION LIMITS
          // For profile, compress to 3.7MB before encryption; for profile2, compress to 2.4MB
          const targetSize = inscriptionType === 'profile' ? 3.7 : 2.4;
        ...
        if (inscriptionType === 'profile2' && backgroundImageFile) {
          ...
          // For profile2, compress to 1.7MB before encryption
          const base64Data = await imageToBase64(backgroundImageFile, 800, true, 1.7);
  ...
  // Fetch inscription content details  handleImageSelect
  ...
  // Handle image selection - now allows up to 10MB with automatic compression
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      setStatus({ 
        type: 'error', 
        message: `Image too large. Maximum size is 10MB, your file is ${(file.size / (1024 * 1024)).toFixed(2)}MB.` 
      });
      return;
    }

    // Determine target size based on encryption and inscription type
    let targetSizeMB = 4.95; // Default for unencrypted images
    if (encryptionLevel > 0) {
      targetSizeMB = 2.4; // Encrypted images compressed to 2.4MB
    }

    // Show compression info if file exceeds target
    const targetSizeBytes = targetSizeMB * 1024 * 1024;
    if (file.size > targetSizeBytes) {
      setStatus({ 
        type: 'info', 
        message: `Image (${(file.size / 1024 / 1024).toFixed(1)}MB) will be automatically compressed to ${targetSizeMB}MB for ${encryptionLevel > 0 ? 'encrypted' : 'unencrypted'} inscription.` 
      });
    } else {
  ...
  const handleProfileImageSelect = async (e: React.ChangeEvent<HTMLInputElement>, isBackground: boolean = false) => {
    ...
    // Determine target sizes based on encryption and inscription type
    let targetSizeMB = 0;
    if (encryptionLevel > 0) {
      // Encrypted sizes
      if (inscriptionType === 'profile') {
        targetSizeMB = 3.7; // Profile encrypted
      } else if (inscriptionType === 'profile2') {
        targetSizeMB = isBackground ? 1.7 : 2.4; // Profile2 encrypted (background: 1.7MB, avatar: 2.4MB)
      }
    } else {
      // Unencrypted sizes
      if (inscriptionType === 'profile') {
        ...
        targetSizeMB = 1.7; // Both avatar and background use 1.7MB when unencrypted
      }
    }
    ...
    if (file.size > targetSizeBytes) {
      setStatus({ 
        type: 'info', 
        message: `Large ${isBackground ? 'background' : 'avatar'} image (${(file.size / 1024 / 1024).toFixed(1)}MB) will be compressed to ${targetSizeMB}MB.` 
      });
    } else {
      setStatus({ 
        type: 'info', 
        message: `${isBackground ? 'Background' : 'Avatar'} image: ${(file.size / 1024).toFixed(0)}KB` 
      });
    }
  };
  ...
  const imageToBase64 = (file: File, maxWidth?: number, isEncrypted: boolean = false, targetSizeMB?: number): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      // COMPRESSION LIMITS CONFIGURATION - MAIN LOGIC
      ...
      if (!isEncrypted && !targetSizeMB) {
        // UNENCRYPTED COMPRESSION LIMITS BY TAB
        if (inscriptionType === 'image') {
          compressionThreshold = 4.95 * 1024 * 1024; // 4.95MB for unencrypted images
          targetSizeMB = 4.95;
        } else if (inscriptionType === 'profile') {
          compressionThreshold = 3.7 * 1024 * 1024; // 3.7MB for unencrypted profile
          targetSizeMB = 3.7;
        } else if (inscriptionType === 'profile2') {
          compressionThreshold = 1.7 * 1024 * 1024; // 1.7MB for unencrypted profile2
          targetSizeMB = 1.7;
        }
      }
        ...
        img.onload = () => {
          ...
          if (!maxDimension) {
            ...
            if (isEncrypted && targetSizeMB) {
              ...
              // Non-encrypted compression logic - UPDATED FOR BETTER QUALITY
              ...
              if (currentDataSize > MAX_SAFE_DATA_SIZE) {
                ...
                const reductionFactor = Math.sqrt(MAX_SAFE_DATA_SIZE / currentDataSize) * 0.95; // 0.95 to ensure we hit target
                maxDimension = Math.max(1200, Math.floor(Math.max(width, height) * reductionFactor)); // Minimum 1200px
              } else {
          ...
          if (isEncrypted && targetSizeMB) {
            ...
            quality = 0.92; // Slightly lower quality for encrypted
          } else if (targetSizeMB) {
            // For unencrypted, aim for 95% of target to ensure we don't exceed
            targetSize = targetSizeMB * 1024 * 1024 * 0.95;
            quality = 0.95; // Higher quality for unencrypted
          }
          
          let dataUrl = canvas.toDataURL(file.type || 'image/jpeg', quality);
          
          // Reduce quality gradually until size is acceptable
          while (dataUrl.length > targetSize && quality > 0.5) { // Don't go below 50% quality
            quality -= 0.03; // Smaller steps for better quality
            dataUrl = canvas.toDataURL('image/jpeg', quality);
          }
          
          // If still too large, reduce dimensions further
          if (dataUrl.length > targetSize && quality <= 0.5) {
            const scaleFactor = Math.sqrt(targetSize / dataUrl.length) * 0.95;
            canvas.width = Math.round(width * scaleFactor);
            canvas.height = Math.round(height * scaleFactor);
            ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
            dataUrl = canvas.toDataURL('image/jpeg', 0.85); // Use 85% quality for final attempt
          }
...
// tried thisfile via compression rates but no joy, ended up with transaction errors. reverted back to orginal 