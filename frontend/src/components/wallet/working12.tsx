...
export const ProfileToken: React.FC = () => {
  ...
  const imageToBase64 = (file: File, forceCompress?: boolean): Promise<string> => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      
      // Determine target sizes based on inscription type and encryption level
      let targetSize: number;
      let maxAllowedSize: number;
      
      if (inscriptionType === 'image') {
        if (encryptionLevel === 0) {
          maxAllowedSize = 4.95 * 1024 * 1024; // 4.95MB
          targetSize = 4.9 * 1024 * 1024; // 4.9MB
        } else {
          maxAllowedSize = 2.4 * 1024 * 1024; // 2.4MB
          targetSize = 2.35 * 1024 * 1024; // 2.35MB
        }
      } else if (inscriptionType === 'profile') {
        maxAllowedSize = 3.7 * 1024 * 1024; // 3.7MB
        targetSize = 3.65 * 1024 * 1024; // 3.65MB
      } else if (inscriptionType === 'profile2') {
        maxAllowedSize = 1.7 * 1024 * 1024; // 1.7MB
        targetSize = 1.65 * 1024 * 1024; // 1.65MB
      } else {
        // Default fallback
        maxAllowedSize = 5 * 1024 * 1024;
        targetSize = 4.5 * 1024 * 1024;
      }
      
      // For base64, we need to adjust the target size (base64 is ~1.37x larger than binary)
      const base64TargetSize = targetSize * 1.37;
      const base64MaxSize = maxAllowedSize * 1.37;
      
      // If file is small enough and we're not forcing compression, just convert directly
      if (file.size < maxAllowedSize && !forceCompress) {
        reader.onload = () => {
          const base64 = reader.result as string;
          const base64Data = base64.split(',')[1];
          
          // Check if base64 is within limits
          if (base64Data.length <= base64MaxSize) {
            resolve(base64Data);
          } else {
            // Need to compress
            compressImage();
        ...
        reader.onload = (e) => {
          ...
          img.onload = () => {
            ...
            const tryCompression = () => {
              ...
              return dataUrl.split(',')[1].length;
            };
            
            // Try initial compression
            let currentSize = tryCompression();
            
            // Reduce dimensions and quality until we meet target
            while (currentSize > base64TargetSize && (width > 100 || quality > 0.1)) {
              ...
              if (quality > 0.3) {
                ...
                quality = 0.85; // Reset quality when scaling
              }
              
              currentSize = tryCompression();
            }
            
            const base64Data = dataUrl.split(',')[1];
            const compressedSizeMB = (base64Data.length * 0.75 / 1024 / 1024).toFixed(2);
            const originalSizeMB = (file.size / 1024 / 1024).toFixed(2);
            
            console.log(`Image compressed: ${originalSizeMB}MB → ${compressedSizeMB}MB (${width}x${height}, quality ${quality.toFixed(2)})`);
            console.log(`Target was ${(targetSize / 1024 / 1024).toFixed(2)}MB, max allowed ${(maxAllowedSize / 1024 / 1024).toFixed(2)}MB`);
    ...
    try {
      ...
      if (encryptionLevel > 0 && encryptedData) {
        ...
        if (inscriptionType === 'text') {
          ...
          if (profileImageFile) {
            const base64Data = await imageToBase64(profileImageFile, true);
            profileDataToSave.avatar = `data:${profileImageFile.type};base64,${base64Data}`;
          }
          
          if (inscriptionType === 'profile2' && backgroundImageFile) {
            const base64Data = await imageToBase64(backgroundImageFile, true);
            profileDataToSave.background = `data:${backgroundImageFile.type};base64,${base64Data}`;
          }
        ...
        <div>
          ...
          <div className="p-4 bg-gray-700 rounded-lg border border-gray-600">
            <div className="space-y-4">
              ...
              {/* Info Box with Updated Compression Rates */}
              <div className="p-3 bg-blue-900 bg-opacity-30 rounded-lg border border-blue-700">
                ...
                <ul className="text-xs text-gray-300 space-y-1">
                  ...
                  <li className="font-medium text-yellow-300">📷 Image Tab Limits:</li>
                  <li className="ml-4">• Unencrypted: 4.95MB max (compressed to 4.9MB if larger)</li>
                  <li className="ml-4">• Encrypted: 2.4MB max (compressed to 2.35MB if larger)</li>
                  <li className="font-medium text-green-300">👤 Profile Tab Limits:</li>
                  <li className="ml-4">• Both encrypted & unencrypted: 3.7MB max (compressed to 3.65MB if larger)</li>
                  <li className="font-medium text-purple-300">🖼️ Profile2 Tab Limits:</li>
                  <li className="ml-4">• Combined avatar + background: 1.7MB max (compressed to 1.65MB if larger)</li>
                  <li className="ml-4">• Same limit for both encrypted & unencrypted</li>
