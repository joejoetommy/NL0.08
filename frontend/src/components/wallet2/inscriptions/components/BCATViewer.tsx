...
export const BCATViewer: React.FC<BCATViewerProps> = ({
    ...
    try {
      ...
      for (let i = 0; i < bcat.chunkTxIds.length; i++) {
        ...
        if (chunkData) {
          ...
          throw new Error(`Failed to extract data from chunk ${i + 1}`);
        }
        
        // Small delay to avoid rate limiting
        if (i < bcat.chunkTxIds.length - 1) {
      ...
      const blob = new Blob([combined], { type: bcat.metadata.mimeType || 'application/octet-stream' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = bcat.metadata.filename || `bcat-file-${Date.now()}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      
      alert(`File reconstructed successfully!\nFilename: ${bcat.metadata.filename || 'bcat-file'}\nSize: ${(totalLength / 1024 / 1024).toFixed(2)}MB`);
  ...
  const extractChunkData = async (txData: any): Promise<Uint8Array | null> => {
    try {
      ...
      const namespacePos = scriptHex.indexOf(BCAT_PART_NAMESPACE_HEX);
      if (namespacePos < 0) return null;
      
      // Parse to find the data after namespace
      ...
      while (pos < scriptHex.length) {
        ...
        if (dataHex === BCAT_PART_NAMESPACE_HEX) {
          // Next push should be the actual data
          pos = dataStart + dataLength * 2;
          
          if (pos + 2 <= scriptHex.length) {
            const dataOpcode = parseInt(scriptHex.substr(pos, 2), 16);
            let chunkLength = 0;
            let chunkStart = pos;
            
            if (dataOpcode <= 75) {
              chunkLength = dataOpcode;
              chunkStart = pos + 2;
            } else if (dataOpcode === 0x4c) {
              chunkLength = parseInt(scriptHex.substr(pos + 2, 2), 16);
              chunkStart = pos + 4;
            } else if (dataOpcode === 0x4d) {
              chunkLength = parseInt(scriptHex.substr(pos + 2, 2), 16) + 
                           (parseInt(scriptHex.substr(pos + 4, 2), 16) << 8);
              chunkStart = pos + 6;
            } else if (dataOpcode === 0x4e) {
              chunkLength = parseInt(scriptHex.substr(pos + 2, 2), 16) + 
                           (parseInt(scriptHex.substr(pos + 4, 2), 16) << 8) +
                           (parseInt(scriptHex.substr(pos + 6, 2), 16) << 16) +
                           (parseInt(scriptHex.substr(pos + 8, 2), 16) << 24);
            ...
            if (chunkStart + chunkLength * 2 <= scriptHex.length) {
              const chunkHex = scriptHex.substr(chunkStart, chunkLength * 2);
              const chunkData = new Uint8Array(chunkLength);
              
              for (let i = 0; i < chunkLength; i++) {
                chunkData[i] = parseInt(chunkHex.substr(i * 2, 2), 16);
              }
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  disabled={reconstructing && selectedBcat?.txid === bcat.txid}
