...
// const CommentItem: React.FC<CommentProps> = ({ report, onReplyClick }) => {
...
//   const { likes, dislikes, tips } = report;
...
//     tip: { clicked: false, count: report.tips },
//     linkOut: { clicked: false, count: 0 }
//   });

//   const [repliesStatus, setRepliesStatus] = useState(report.replies.map(reply => ({
//     id: reply.id,
//     likeClicked: false,
//     dislikeClicked: false,
//     likes: reply.likes,
...
//   }, [report.replies, showReplies]);

//   const toggleReplies = () => {
//     setShowReplies(!showReplies);
//     if (!showReplies) {
//       setDisplayedReplies(report.replies.slice(0, repliesPerPage));
...
//     const additionalReplies = report.replies.slice(
...
//           alt={`${report.user}'s avatar`}
//           className="w-8 h-8 rounded-full"
//         />
//         <div className="flex-1">
//           <div className="flex justify-between items-center">
//             <span className="font-bold text-gray-300 cursor-pointer" onClick={() => alert('Redirect to profile')}>{report.user}</span>
...
//         <button className="text-sm" onClick={() => onReplyClick(report.user)}>Reply</button>
...
//             {showReplies ? 'Hide replies' : `View replies (${formatCount(report.replies.length)})`}
//           </button>
//           {showReplies && (
//             <>
...
//                 </div>
//               ))}
//               {report.replies.length > repliesPageIndex * repliesPerPage && (
...
// const DrawerReport: React.FC<DrawerReportProps> = ({ reports }) => {
//   const [placeholder, setPlaceholder] = useState("Add report here ...");
...
//         {reports.map(report => (
//           <CommentItem key={report.id} report={report} onReplyClick={handleReplyClick} />
//         ))}
//       </div>
