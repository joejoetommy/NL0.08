...
export const ProfileToken: React.FC = () => {
  ...
  // Backend proxy URL - adjust this to match your setup
  const BROADCAST_PROXY_URL = process.env.REACT_APP_BROADCAST_PROXY_URL || 'http://localhost:3001';
  ...
  const broadcastTransaction = async (txHex: string): Promise<{ success: boolean; txid?: string; error?: string }> => {
    try {
      ...
      // Fallback: show transaction hex for manual broadcast
      console.log('Transaction hex for manual broadcast:');
      console.log(txHex);
      
      return {
        success: false,
        error: `Network error. Check console for transaction hex. Manual broadcast at: https://${network === 'testnet' ? 'test.' : ''}whatsonchain.com/broadcast`
      };
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-gray-800 rounded-lg">
            <p className="text-xs text-gray-400">
              ...
              <br />
              <span className="text-yellow-400">Make sure the broadcast proxy server is running!</span>
            </p>
          </div>
        </div>
      </div>
