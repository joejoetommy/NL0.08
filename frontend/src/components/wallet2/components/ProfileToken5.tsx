...
//         // For encrypted images, compress to 4.9MB before encryption
//         const base64Data = await imageToBase64(imageFile, undefined, true, 4.9);
...
export const ProfileToken: React.FC = () => {
  ...
  const encryptCurrentData = async () => {
    ...
    try {
      ...
      if (inscriptionType === 'text') {
        ...
        // For encrypted images, enforce 4.9MB limit
        const base64Data = await imageToBase64(imageFile, undefined, true, 4.9);
        ...
        if (profileImageFile) {
          ...
          // For profile, enforce 3.7MB limit when encrypted
          const targetSize = inscriptionType === 'profile' ? 3.7 : 2.4;
          const base64Data = await imageToBase64(profileImageFile, 600, true, targetSize);
          profileDataToSave.avatar = `data:${profileImageFile.type};base64,${base64Data}`;
        }
        ...
        if (inscriptionType === 'profile2' && backgroundImageFile) {
          ...
          // For profile2, enforce 1.7MB limit when encrypted  Please use a smaller image
          const base64Data = await imageToBase64(backgroundImageFile, 800, true, 1.7);
          profileDataToSave.background = `data:${backgroundImageFile.type};base64,${base64Data}`;
        }
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const base64Size = Math.ceil(file.size * 1.37); // Base64 overhead
    const { estimatedSize, fee, remainingCapacity } = calculateTransactionFee(1, 2, base64Size);
    
    // Check if transaction would exceed 5MB
    if (estimatedSize > 5 * 1024 * 1024) {
      setStatus({ 
        type: 'error', 
        message: `Transaction would be too large (${(estimatedSize / 1024 / 1024).toFixed(2)}MB). Please use a smaller image.` 
      });
      return;
    }
    ...
    setStatus({ 
      type: 'info', 
      message: `Image size: ${(file.size / 1024).toFixed(0)}KB. Estimated transaction: ${(estimatedSize / 1024 / 1024).toFixed(2)}MB. Fee: ${fee} sats` 
    });
  ...
  const imageToBase64 = (file: File, maxWidth?: number, isEncrypted: boolean = false, targetSizeMB?: number): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      if (file.size < 300000 && !maxWidth && !isEncrypted) { // < 300KB
        reader.onload = () => {
          const base64 = reader.result as string;
          const base64Data = base64.split(',')[1];
          resolve(base64Data);
        ...
        img.onload = () => {
          ...
          if (!maxDimension) {
            ...
            if (isEncrypted && targetSizeMB) {
              ...
              // Non-encrypted compression logic
              const MAX_SAFE_DATA_SIZE = 4.5 * 1024 * 1024; // 4.5MB to be safe
              const currentDataSize = file.size * 1.37; // Base64 overhead
          ...
          if (isEncrypted && targetSizeMB) {
            targetSize = targetSizeMB * 1024 * 1024 * 0.9; // 90% of target to be safe
            quality = 0.85;
          } else if (maxWidth || file.size > 2000000) {
            targetSize = 2500000; // Target ~2.5MB base64 for encrypted images
            quality = 0.85;
          }
          ...
          // Final check for transaction size
          const estimatedTxSize = base64Data.length + 500; // Add overhead
          if (estimatedTxSize > 4.8 * 1024 * 1024) { // Leave margin for 5MB limit
            reject(new Error(`Compressed image still too large for transaction: ${(estimatedTxSize / 1024 / 1024).toFixed(1)}MB`));
            return;
          }
